...
- [x] All three assignment strategies validated, including incremental (cooperative-sticky) reassignments and stale-ack guards.
  ...
  - Not covered end-to-end (mock limitation, investigated): surfacing a commit failure (`offset_commit_errors`) via injected `OffsetCommit` faults. On `MockCluster` async offset-commit outcomes are not delivered to the receiver's `commit_callback` within a test window -- even a successful commit that reaches the broker leaves the counters at 0 -- so the injected fault cannot be observed as `offset_commit_errors >= 1`. This is a mock/timing limitation, not a receiver defect (see the Section 1 async-commit note for the full analysis). The commit-error *folding* is validated deterministically by the unit test `reconcile_folds_commit_callback_metrics`, and the shutdown fold was hardened (`reconcile_rebalance_state()` before the terminal snapshot). A real-broker end-to-end variant is deferred.
