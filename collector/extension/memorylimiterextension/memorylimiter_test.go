...
func (mockmle *mockMemoryLimiterExtension) UnaryInterceptorGenerator() grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (resp any, err error) {
		inner := mockmle.memoryLimiterExtension.UnaryInterceptorGenerator()
...
type grpcTraceClient struct {
	ptraceotlp.GRPCClient
	blockCh chan struct{}
}

func (gtc *grpcTraceClient) Export(ctx context.Context, req ptraceotlp.ExportRequest) (ptraceotlp.ExportResponse, error) {
	<-gtc.blockCh
	return gtc.GRPCClient.Export(ctx, req)
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.numIter; i++ {
				...
				go func() {
					...
					resp, errResp := basecl.Export(ctx, ptraceotlp.NewExportRequestFromTraces(traces))
