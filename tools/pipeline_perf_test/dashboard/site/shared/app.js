    ...
    for (let dsIdx = 0; dsIdx < chart.data.datasets.length; dsIdx++) {
      ...
      for (let i = 0; i < meta.data.length; i++) {
        ...
        if (flags[i]) {
          ctx.font = font;
          const icon = "\u26A0 ";
          const fw = ctx.measureText(icon + label).width;
          const iw = ctx.measureText(icon).width;
          const sx = el.x - fw / 2;
          ctx.textAlign = "left";
          ctx.fillStyle = "#ef4444";
          ctx.fillText(icon, sx, el.y - 4);
          ctx.fillStyle = "#64748b";
          ctx.fillText(label, sx + iw, el.y - 4);
        } else {
          ctx.font = font;
          ctx.fillStyle = "#64748b";
...
function getColor(index) { return AUTO_COLORS[index % AUTO_COLORS.length]; }
...
const TIMESERIES_METRICS = [
  ...
  { key: "ram_mib", label: "RAM (MiB)", unit: "MiB", avg: "ram_mib_avg", max: "ram_mib_max" },
  ...
  function renderChart() {
    ...
    if (canvas && filtered.suites.length > 0) {
      const onClick = (event, elements) => {
        ...
        const ref = (filtered.suites || [])[datasetIndex];
        const tn = testNames[index];
        if (ref && tn) openTestModal(suiteData, ref, tn);
      };
      activeCharts.set(slug, createBarChart(canvas, suiteData, filtered, testNames, sel, onClick));
    }
...
function renderComparisonChart(suiteData, comparison, testNames, onBarClick) {
  ...
  target.innerHTML = `
    <div class="scenario-section">
      <div class="scenario-section-head">
        <div class="scenario-section-title">${escapeHtml(metricLabel(sel))}</div>
  ...
  if (ms) ms.onchange = () => {
    ...
    if (t) t.textContent = metricLabel(sel);
  };
}
...
async function openTestModal(suiteData, suiteRef, testName) {
  const modal = document.getElementById("run-detail-modal");
  const body = document.getElementById("run-detail-body");
  const title = document.getElementById("run-detail-title");
  if (!modal || !body || !title) return;
  const test = getTestByName(suiteData, suiteRef.slug, testName);
  title.textContent = `${suiteRef.short || suiteRef.name} — ${testName}`;
  modal.hidden = false;
  if (!test) { body.innerHTML = '<div class="muted">No data available for this test.</div>'; return; }

  const metrics = test.metrics || [];
  const allFiles = [...(test.configFiles || [])].sort();
  const metricBarsHtml = metrics.length
    ? `<div class="run-metric-grid">${metrics.filter((m) => m.value != null).map((m) => `<div class="run-metric-bar"><div class="run-metric-head"><div><div class="run-metric-name">${escapeHtml(metricLabel(m.name))}</div></div><div class="run-metric-value">${formatMetricValue(m.value, m.unit || "")}</div></div><div class="run-metric-track"><div class="run-metric-fill" style="width:${Math.min(100, Math.max(2, 50))}%"></div></div></div>`).join("")}</div>`
    : '<div class="muted">No metrics available.</div>';

  const filesHtml = allFiles.length
    ? `<div class="modal-section"><div class="config-tab-bar"><label for="config-file-select">File</label><select id="config-file-select" class="config-select">${allFiles.map((f) => `<option value="${escapeHtml(f)}">${escapeHtml(f)}</option>`).join("")}</select></div><pre id="config-full-viewer" class="config-full-code"><code id="config-full-code-content"></code></pre></div>`
    : '<div class="muted">No files available.</div>';

  body.innerHTML = `
    <div class="modal-tabs"><button class="modal-tab active" data-modal-tab="metrics" type="button">Metrics</button><button class="modal-tab" data-modal-tab="raw" type="button">Files</button></div>
    <div class="modal-tab-panel" data-modal-panel="metrics"><div class="modal-section"><div class="modal-section-title">Test Metrics</div>${metricBarsHtml}</div></div>
    <div class="modal-tab-panel" data-modal-panel="raw" hidden>${filesHtml}</div>`;

  for (const tab of body.querySelectorAll("[data-modal-tab]")) tab.onclick = () => {
    for (const t of body.querySelectorAll("[data-modal-tab]")) t.classList.toggle("active", t === tab);
    for (const p of body.querySelectorAll("[data-modal-panel]")) p.hidden = p.dataset.modalPanel !== tab.dataset.modalTab;
  };

  const fileSelect = document.getElementById("config-file-select");
  const codeEl = document.getElementById("config-full-code-content");
  async function showFile(fn) {
    if (!codeEl) return;
    codeEl.textContent = "Loading...";
    try { codeEl.innerHTML = highlightFileContent(fn, await loadConfigFile(suiteRef.slug, testName, fn)); }
    catch (e) { codeEl.textContent = `Error loading file: ${e.message}`; }
  }
  if (fileSelect && allFiles.length) { fileSelect.onchange = () => showFile(fileSelect.value); showFile(allFiles[0]); }
}
