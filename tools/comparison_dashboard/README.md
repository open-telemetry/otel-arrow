...
Pages source must be set to "Deploy from a branch: `benchmarks`" so that
the existing `benchmark-action/github-action-benchmark` flow can keep
publishing under `docs/benchmarks/`.
