...
#[test]
fn debug_resource_spans_size() {
    let varint_size = |n: u64| -> usize {
        if n < 0x80 {
            1
        } else if n < 0x4000 {
            2
    ...
    let traces_data = TracesData::new(rss.clone());

    println!("🔍 Debugging ResourceSpans size calculation:");
    println!("pdata_size(): {}", traces_data.pdata_size());
    println!("encoded_len(): {}", traces_data.encoded_len());
    println!(
        "Difference: {}",
        traces_data.encoded_len() as i32 - traces_data.pdata_size() as i32
    );

    // Break down by components
    let resource_spans = &traces_data.resource_spans[0];
    println!("\n📊 Component breakdown:");
    println!(
        "Resource: pdata={}, prost={}",
        resource_spans.resource.as_ref().unwrap().pdata_size(),
        resource_spans.resource.as_ref().unwrap().encoded_len()
    );

    let scope_spans = &resource_spans.scope_spans[0];
    println!(
        "ScopeSpans: pdata={}, prost={}",
        scope_spans.pdata_size(),
        scope_spans.encoded_len()
    );
    ...
    // Break down ResourceSpans level
    println!("\n📊 ResourceSpans breakdown:");
    println!(
        "ResourceSpans[0]: pdata={}, prost={}",
        resource_spans.pdata_size(),
        resource_spans.encoded_len()
    );
    println!(
        "ResourceSpans[1]: pdata={}, prost={}",
        traces_data.resource_spans[1].pdata_size(),
        traces_data.resource_spans[1].encoded_len()
    );

    // Debug ResourceSpans[0] fields
    println!("\n🔍 ResourceSpans[0] field breakdown:");
    if let Some(resource) = &resource_spans.resource {
        println!(
            "  resource: pdata={}, prost={}",
            resource.pdata_size(),
            resource.encoded_len()
        );
    } else {
        println!("  resource: None");
    }

    println!("  scope_spans count: {}", resource_spans.scope_spans.len());
    for (i, scope_span) in resource_spans.scope_spans.iter().enumerate() {
        println!(
            "  scope_spans[{}]: pdata={}, prost={}",
            i,
            scope_span.pdata_size(),
            scope_span.encoded_len()
        );
    }

    println!("  schema_url: {:?}", resource_spans.schema_url);

    // Manual calculation of ResourceSpans[0]
    let resource_size = resource_spans
        .resource
        .as_ref()
        .map_or(0, |r| r.encoded_len());
    let scope_spans_size: usize = resource_spans
        .scope_spans
        .iter()
        .map(|ss| {
            let ss_size = ss.encoded_len();
            1 + varint_size(ss_size as u64) + ss_size // tag + length + value for each scope_spans entry
        })
        .sum();
    let schema_url_size = if resource_spans.schema_url.is_empty() {
        0
    } else {
        let len = resource_spans.schema_url.len();
        1 + varint_size(len as u64) + len // tag + length + value for schema_url
    };

    let rs_manual = if resource_size > 0 {
        1 + varint_size(resource_size as u64) + resource_size // tag + length + value for resource
    } else {
        0
    } + scope_spans_size
        + schema_url_size;
    ...
    println!(
        "  Manual ResourceSpans[0] calc: resource({}) + scope_spans({}) + schema_url({}) = {}",
        if resource_size > 0 {
            1 + varint_size(resource_size as u64) + resource_size
        } else {
            0
        },
        scope_spans_size,
    ...
    // Manual calculation of repeated field
    let rs0_size = resource_spans.encoded_len();
    let rs1_size = traces_data.resource_spans[1].encoded_len();
    let manual_total =
        1 + varint_size(rs0_size as u64) + rs0_size + 1 + varint_size(rs1_size as u64) + rs1_size;
    println!(
        "Manual calculation: tag+len+value for each = {}",
        manual_total
    );

    // Debug the repeated field calculation specifically
    let scope_spans = &resource_spans.scope_spans;
    println!("🔍 ScopeSpans repeated field debug:");
    println!("Number of items: {}", scope_spans.len());

    // Manual calculation of repeated field
    let mut manual_repeated_total = 0;
    for (i, scope_span) in scope_spans.iter().enumerate() {
        let item_size = scope_span.encoded_len();
        let tag_size = varint_size(2 << 3 | 2); // tag 2, wire_type 2 (length-delimited)
        let len_size = varint_size(item_size as u64);
        let total_item = tag_size + len_size + item_size;
        println!(
            "  ScopeSpans[{}]: item_size={}, tag_size={}, len_size={}, total={}",
            i, item_size, tag_size, len_size, total_item
        );
        manual_repeated_total += total_item;
    }
    println!("Manual repeated total: {}", manual_repeated_total);

    // Compare with individual scope_spans calculations
    for (i, scope_span) in scope_spans.iter().enumerate() {
        println!(
            "ScopeSpans[{}]: pdata={}, prost={}",
            i,
    ...
    // Check individual span fields
    println!("\n🔍 Span field breakdown:");
    println!("trace_id length: {}", span.trace_id.len());
    println!("span_id length: {}", span.span_id.len());
    println!("flags: {}", span.flags);
    println!("start_time_unix_nano: {}", span.start_time_unix_nano);
    println!("end_time_unix_nano: {}", span.end_time_unix_nano);

    // Create a simple ResourceSpans with just one scope_spans to isolate the issue
    println!("\n🔬 ISOLATION TEST: Single ScopeSpans");
    let simple_rs = ResourceSpans::build(Resource::new(vec![]))
        .scope_spans(vec![scope_spans.iter().next().unwrap().clone()])
        .finish();

    println!(
        "Simple ResourceSpans: pdata={}, prost={}",
        simple_rs.pdata_size(),
        simple_rs.encoded_len()
    );
    println!(
        "Expected difference for single item: {}",
        simple_rs.encoded_len() as i32 - simple_rs.pdata_size() as i32
    );

    // Manual calculation for single ScopeSpans
    let single_scope_span = scope_spans.iter().next().unwrap();
    let item_size = single_scope_span.encoded_len();
    let tag_size = varint_size((2 << 3 | 2) as u64); // tag 2, wire_type 2
    let len_size = varint_size(item_size as u64);
    let expected_single = tag_size + len_size + item_size;

    println!(
        "Manual single calc: tag={}, len={}, item={}, total={}",
        tag_size, len_size, item_size, expected_single
    );

    // Check if prost's encoded_len_repeated gives different result
    println!("item_size from scope_span.encoded_len(): {}", item_size);
    println!(
        "ScopeSpans content: scope={:?}, spans.len()={}",
        single_scope_span.scope.as_ref().map(|s| &s.name),
        single_scope_span.spans.len()
    );

    assert_eq!(
        traces_data.pdata_size(),
        traces_data.encoded_len(),
        "Size mismatch in ResourceSpans"
    );
