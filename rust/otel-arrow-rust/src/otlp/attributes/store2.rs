...
#[derive(Default)]
pub struct AttributeStore2<'a, T> {
    /// The RecordBatch containing attribute data
    pub record_batch: &'a RecordBatch,
    ...
    T: ParentId + Clone,
{
    ...
    pub fn attributes_by_id(&self, id: &T) -> Option<AttributeIterator<T>> {
        self.first_index_by_id
            .get(id)
            .map(|&first_idx| AttributeIterator {
                store: self,
    ...
    T: ParentId + Clone,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let key = key_arr
            .and_then(|arr| arr.value_at(idx))
            .unwrap_or_default();
        let value_type =
            AttributeValueType::try_from(value_type_arr.value_at_or_default(idx)).ok()?;

        let value_str_arr =
            StringArrayAccessor::try_new_for_column(rb, consts::ATTRIBUTE_STR).ok()?;
        ...
        let value = match value_type {
            ...
            AttributeValueType::Bool => Value::BoolValue(value_bool_arr.value_at_or_default(idx)),
    ...
    <<T as ParentId>::ArrayType as ArrowPrimitiveType>::Native: Into<T>,
{
    ...
    fn try_from(rb: &RecordBatch) -> Result<Self, Self::Error> {
        ...
        let mut store = Self { record_batch: rb };
...
mod tests {
    use super::*;
    use arrow::array::{
        BinaryArray, BooleanArray, Float64Array, Int64Array, StringArray, UInt8Array,
    };
    ...
    fn create_test_batch() -> RecordBatch {
        ...
        let types = Arc::new(UInt8Array::from(vec![1, 2, 1, 3, 4, 2, 1, 1]));
