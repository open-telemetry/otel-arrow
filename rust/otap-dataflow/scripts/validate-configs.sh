...
# otap-dataflow tree and runs the engine binary with --validate-config-only to
# verify that each config is structurally valid, references only known plugins,
# and has correct plugin-specific configuration.
#
# Usage:
...
# optional plugin features enabled and uses that.
...
if [ $# -ge 1 ]; then
    ...
    echo "Building df_engine with all plugin features..."
    cargo build --release \
        --features experimental-tls,experimental-exporters,experimental-processors,recordset-kql-processor,azure-monitor-exporter,geneva-exporter,condense-attributes-processor,resource-validator-processor \
...
for config in $CONFIG_FILES; do
    ...
    if "$BINARY" --config "$config" --validate-config-only > /dev/null 2>&1; then
        echo "  ✅ $REL_PATH"
        PASSED=$((PASSED + 1))
    else
        echo "  ❌ $REL_PATH"
        # Re-run to capture the error message
        "$BINARY" --config "$config" --validate-config-only 2>&1 | tail -5 | sed 's/^/     /'
        FAILED=$((FAILED + 1))
