...
/// null attributes.
fn read_logs_batch(batch_size: usize, with_nulls: bool) -> OtapArrowRecords {
    let mut bytes = Vec::new();
    let mut file =
...
fn bench_transport_encode_id_for_attributes(c: &mut Criterion) {
    ...
    for size in [0, 128, 1536, 8092] {
        ...
        // TODO - honestly probably not necesary to decode these first
        let input = materialize_parent_id_for_attributes::<u16>(&input)
            .expect("function should not error here");

        let _ = group.bench_with_input(
            BenchmarkId::new("transport_encode_parent_id_for_attributes", size),
            &input,
            |b, input| {
                b.iter_batched(
                    || input.clone(),
...
fn bench_decode_transport_optimized_ids(c: &mut Criterion) {
    ...
    for size in [127, 1536, 8096] {
        ...
        for with_nulls in [false, true] {
            ...
            println!("TODO ALBERT YOU REMOVED ENCODE CALL FROM DECODE TEST");
            //
            // otap_batch
...
mod benches {
    ...
    criterion_group!(
        ...
        targets = bench_materialize_parent_ids,
            ...
            bench_transport_encode_id_for_attributes
    );
}
