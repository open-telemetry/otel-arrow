...
- **Instance policy (Cloned vs Fresh).** A capability
  consumer receives either a clone of a stored prototype
  (`.cloned()`) or a freshly-constructed instance from a
  user-supplied closure (`.fresh()`). The policy is chosen
  by the extension author at build time and is invisible
  to consumers -- `require_shared()` / `require_local()`
  return the same trait-object type regardless. Fresh is
  Passive-only; the combination `Active + Fresh` is
  unrepresentable in the typestate builder (an Active
  extension has a single engine-driven event loop that
  doesn't compose with per-consumer construction).
...
4. **Instance policy (Passive only).** After `.passive()`,
   ...
   - `.fresh()` -- each consumer receives a
     freshly-constructed instance from a user-supplied
     `Fn() -> E + Clone` closure.

   `Active + Fresh` is unrepresentable: `ActiveStage`
   exposes no `.fresh()` method, so the invalid
...
5. **Local/Shared split.** Each lifecycle/policy stage
   ...
   - **Passive fresh:** `.passive().fresh().shared(|| MyExt::new(cfg.clone())).build()`
...
`Active + Fresh` is compile-time-prevented: `ActiveStage`
exposes no `.fresh()` method.
...
| `.passive().fresh().shared(f)`    | `Box::new(f())` -- user constructor fires  |
| `.passive().fresh().local(f)`     | `f()` -- user constructor fires            |
...
### Passive Extension (Fresh-Per-Consumer)

```rust
// Factory -- each consumer gets a freshly-constructed instance
ExtensionBundleBuilder::new(node, node_config, ext_config)
    .passive()
    .fresh()
...
consumer set does not subsume `.cloned()` vs `.fresh()`:
for any scope with more than one consumer, the provider
still has to answer "when N consumers in this scope ask,
do they get clones of one prototype or independent fresh
instances?" The two axes compose:

| Scope             | Consumer set | `.cloned()`                        | `.fresh()`                          |
| ----------------- | ------------ | ---------------------------------- | ----------------------------------- |
| Node (single)     | 1 consumer   | trivially = fresh                  | trivially = cloned                  |
