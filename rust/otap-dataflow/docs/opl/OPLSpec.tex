...
A {\em scope} is a mapping from names to entities such as variables [\ref{letBindings}, function declarations [\ref{functions}] or pipelines [\ref{pipelines}]. Scopes exist at both compile-time and runtime. An OPL program always has a {\em global compile-time scope}.
...
A {\em let binding} binds a policy [\ref{policies}],  pipeline [\ref{pipelines}], a port tied to an externally declared stream or a value of a basic type to a {\em variable} denoted by an identifier.
...
A {\em port} may be either a {\em source port} or a {\em destination port}.  A source port is indicated by the keyword \RECEIVER{}, and destination port is indicated by the keyword \EXPORTER. Ports are referenced by their declared name. 
...
The port is followed by a block that indicates the configuration of the port, given as a list of key-value pairs, known as {\em attributes}. This specification does not assign any specific meaning to any given attribute; the interpretation of the attributes in a port configuration is determined by the resource or process  bound to the port. Ports are bound to resources outside the OPL program.

{\em See discussion above. }
...
The parameter declarations each include the name of a parameter, $p_i$ and may optionally declare a type $T_i$ for the parameter, $i \in 1 \ldots k$.
...
Let $bp \triangleq$ \POLICY{} \TIMEOUT{} \LCURLY
  \OPERATION\COLON{}$d_1$ \COMMA \PIPELINE\COLON{} $d_2$ \COMMA \ONTIMEOUT\COLON{} $ta$  \RCURLY{} be a timeout policy.
  
  If $ta = \FAIL$ then  if any operation governed by $tp$ takes longer than $d_1$ an $OperationTimeout$ runtime error occurs. The error should provide the name and of the operation that failed, its source code location and the timeout period that was exceeded.       
...
{\em The purpose of \EXTEND{} is to add or mutate dynamic attributes of a stream eleement.}

{\em Or, equivalently, to an element of a list of key-value pairs.}
...
\RENAME{} can also be referenced using the predefined alias \PROJECTRENAME.
...
Let $Stream \lt T \gt$ be the type of $s$.  It is a runtime error if $id$ is not a member of  $T$.

\comment{Even though we give a type warning at compile time, in an optional type system we do not preclude the code from being run (say, for test purposes). If it runs, it must fail at this point.
}
...
The \FORK{} combinator feeds the elements of its input stream into two or more \comment{The original syntax said it limited to two, but other parts of the doc contradict that, and it doesn't seem to be necessary, or a good fit with the rest of the system. Is there a real reason to limit?} child pipelines.
...
An {\em index expression} provides random access to the members of a map  [\ref{attributeTypes}].
...
Let $T_1$ be the type of $e_1$. It is a type warning if $T_1$ is not a map type.
Let $T_2$ be the type of $e_2$. It is a type warning if $T_2$ is not compatible with type \STRING.
...
$Log$s inherit all the fields of $Signal$. In addition a Log has a mandatory filed $time\_unix\_nano$, which is a an $int64$ [\ref{integerTypes}] value indicating the time stamp of the log entry in nanoseconds \comment{since when?}. If a $Log$ value denotes an event, it must also have an $event\_name$ field of type \STRING.  
...
$Metric$s inherit all the fields of $Signal$. In addition a $Span$ has the following mandatory fields: $\{name: \STRING, type: byte\}$.
