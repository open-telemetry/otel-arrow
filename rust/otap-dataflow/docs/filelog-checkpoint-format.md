...
Complete binary artifacts are stored in the checked-in, tab-separated
[version 1 vector fixture](filelog-checkpoint-v1-vectors.tsv). Each data row
contains a stable vector name, the required decode result, and lowercase hex.
Consumers must decode the hex directly; the fixture is not generated by
round-tripping the encoder under test.
...
namespace, cross-version, cross-platform, publication, and recovery tests must
remain in lockstep with this specification, the fixture, and the
framing-profile compatibility vectors above.
