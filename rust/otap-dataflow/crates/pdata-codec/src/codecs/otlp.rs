...
use crate::{
    ...
    PDATA_CODEC_FACTORIES, PdataDecoder, PdataEncoder, PdataEncoding,
};
...
static OTLP_METADATA: CodecMetadata = CodecMetadata {
...
static OTLP_CODEC: CodecRegistration = CodecRegistration {
    metadata: &OTLP_METADATA,
    decoder: Some(|| Box::new(OtlpDecoder)),
    encoder: Some(|policy| Ok(Box::new(OtlpEncoder::new(policy)))),
    count_items: Some(|signal, bytes| Some(count_items(signal, bytes))),
};
