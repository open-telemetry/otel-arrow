            ...
            .run_test(|mut ctx| async move {
                ...
                for batch in &emitted {
                    if let OTLPData::Logs(req) = batch {
                        ...
                        // NOTE: Gave up on checking resource IDs because this behavior is OK in OTLP
                        //       If we want to enforce resource boundaries, we need some additional logic
                        //       to ensure that log records from different resources are never mixed.
                        // This is a design choice in OTLP, where logs can be mixed across resources
                        //       as long as scopes are preserved.
                        // assert!(
                        //     resource_ids.len() == 1,
