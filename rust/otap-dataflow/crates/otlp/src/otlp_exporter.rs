...
impl OTLPExporter {
    ...
    pub fn new(grpc_endpoint: String) -> Self {
        OTLPExporter { grpc_addr: grpc_endpoint }
    }
}
#[async_trait(?Send)]
impl Exporter<OTLPRequest, SendEffectHandler<OTLPRequest>> for OTLPExporter {
    ...
    async fn start(
        ...
        let mut metrics_client = MetricsServiceClient::connect(self.grpc_endpoint.clone()).await.expect("Couldn't connect");
        let mut logs_client = LogsServiceClient::connect(self.grpc_endpoint.clone()).await.expect("Couldn't connect");
        let mut traces_client = TraceServiceClient::connect(self.grpc_endpoint.clone()).await.expect("Couldn't connect");
...
mod tests {
    ...
    fn test_exporter_with_send_effect_handler() {
        ...
        let (receiver, sender) = tokio::sync::mpsc::channel(32);
        ...
        run_mock_server(sender: sender, listening_addr: addr);
