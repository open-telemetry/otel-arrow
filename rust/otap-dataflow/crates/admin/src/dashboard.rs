...
pub(crate) const CACHE_CONTROL_NO_STORE: &str = "no-store, no-cache, must-revalidate";
...
pub(crate) const CONTENT_SECURITY_POLICY: &str = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self' data:; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'";
...
pub(crate) const X_CONTENT_TYPE_OPTIONS_NO_SNIFF: &str = "nosniff";
...
pub(crate) const X_FRAME_OPTIONS_DENY: &str = "DENY";
...
pub(crate) const REFERRER_POLICY_NO_REFERRER: &str = "no-referrer";
