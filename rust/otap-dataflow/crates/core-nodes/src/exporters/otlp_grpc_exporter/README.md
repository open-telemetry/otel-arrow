...
| Metric | Unit | Attributes | Description |
| --- | --- | --- | --- |
| `exporter.otlp_grpc.authentication` | `{attempt}` | `source` | Auth credential polls that did not produce a usable credential, including failures before a signal batch is admitted. |
