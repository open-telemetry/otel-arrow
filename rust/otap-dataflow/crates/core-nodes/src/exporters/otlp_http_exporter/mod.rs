...
mod test {
    ...
    fn test_handles_invalid_otap_payloads() {
        // In this branch, malformed structured OTAP payloads are rejected when they are inserted
        // into `OtapArrowRecords`, so the exporter never receives an invalid `Logs` batch.
        // Keep this coverage here because the intended malformed shape is still the same one that
        // later branches exercise during exporter-side encoding.
