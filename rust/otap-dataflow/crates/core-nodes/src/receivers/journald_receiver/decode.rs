...
/// Reference decode used by the out-of-crate `journald_decode` benchmark to
/// quantify the win from this change. It runs the real
/// [`decode_journal_fields`] and, when `clone_body` is set, additionally clones
/// the body field's value — exactly the extra per-entry work the pre-change
/// `current_entry` performed (`message_body = Some(value.clone())`). Timing it
/// with `clone_body` `false` versus `true` isolates the eliminated clone.
///
/// Returns a length derived from the result so the caller can `black_box` it and
/// the optimizer cannot elide the work. Exposed only behind the `bench` feature
/// (and under `cfg(test)`); it is not part of the crate's public API.
#[cfg(any(test, feature = "bench"))]
#[must_use]
pub fn bench_reference_decode(raw: &[&[u8]], clone_body: bool) -> usize {
    ...
    let mut acc = decoded.fields.len();
    if clone_body {
        if let Some(index) = decoded.message_body_index {
            // Reproduce the pre-change per-entry work: clone the MESSAGE body.
            // `black_box` the clone so the optimizer cannot prove it dead and
            // elide the allocation+copy under LTO — which would silently defeat
            // the measurement (`body.len()` alone equals `value.len()`, so a
            // naive version optimizes the clone away for large messages).
            let body = decoded.fields[index].value.clone();
            acc = acc.wrapping_add(body.len());
            let _ = std::hint::black_box(body);
        }
    }
    // Force the decoded buffers (including each field's copied value) to be
    // materialized in BOTH variants, so the shared base decode cost is measured
...
mod tests {
    ...
    #[test]
    fn bench_reference_decode_exercises_both_paths() {
        ...
        let index_only = bench_reference_decode(&raw, false);
        let with_clone = bench_reference_decode(&raw, true);
        assert_eq!(index_only, 2);
        // The clone path adds the 2-byte MESSAGE body length.
        assert_eq!(with_clone, 4);
    }
}
