...
| `receiver.received.payload.size` | Counter | `By` | Optional encoded payload bytes visible before parsing, excluding the TCP newline delimiter, grouped by terminal `outcome`. | `crates/core-nodes/src/receivers/syslog_cef_receiver/mod.rs` |
