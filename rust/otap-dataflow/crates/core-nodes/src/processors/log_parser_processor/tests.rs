...
fn parsing_metric_points(registry: &TelemetryRegistryHandle) -> Vec<ParserMetricPoint> {
    ...
    for skip in [false, true] {
        ...
        runtime
            ...
            .run_test(move |mut ctx| async move {
                ...
                let root = RecordBatch::try_new(Arc::new(Schema::new(fields)), columns).unwrap();
...
fn test_parser_to_opl_pipeline() {
    ...
    runtime
        ...
        .run_test(move |mut ctx| async move {
            ...
            let batch = OtapArrowRecords::try_from_with_default(output.clone().payload()).unwrap();
...
struct ParserMetricPoint {
    field: String,
    attributes: BTreeMap<String, String>,
    value: u64,
}

fn parser_metric_points(telemetry_registry: &TelemetryRegistryHandle) -> Vec<ParserMetricPoint> {
...
fn parser_metric_value(
    points: &[ParserMetricPoint],
    field: &str,
...
fn try_create_with_config(
    ...
    let pipeline_context =
        controller_context.pipeline_context_with("group_id".into(), "pipeline_id".into(), 0, 1, 0);
...
fn test_parse_logs_framed_batches() {
    for (format, input, body, text, number) in [
        ...
        let node = include_str!("README.md")
            .split("```yaml")
            .skip(1)
            .map(|block| serde_yaml::from_str::<Value>(block.split("```").next().unwrap()).unwrap())
            ...
            .run_test(move |mut ctx| async move {
                ...
                for permanent in [false, true] {
                    let nack = if permanent {
                        ...
                        NackMsg::new_with_cause("downstream", output[0].clone(), NackCause::Refused)
                    };
                ...
                for index in [0, 2] {
                    let record = &mut expected.resource_logs[0].scope_logs[0].log_records[index];
...
fn test_parse_logs_configuration_gate() {
    let runtime = TestRuntime::<OtapPdata>::new();
    assert!(try_create_with_config(json!({"opl_query":"logs | where true"}), &runtime).is_err());
    let mut config = parsing_config("json");
    ...
    for (timestamp, event, observed, policy, severity, expected_time, reason) in [
        ...
        runtime
            ...
            .validate(move |_ctx| async move {
                ...
                let operations = parser_metric_points(&registry);
                assert_eq!(
                    parser_metric_value(
                        &operations,
                        "operations",
...
fn test_parse_logs_non_log_passthrough() {
    ...
    runtime
        ...
        .validate(move |_ctx| async move {
            ...
            assert!(parser_metric_points(&registry).is_empty());
        });
}
...
fn test_parse_logs_internal_failure_after_staging() {
    ...
    runtime
        ...
        .validate(move |_ctx| async move {
            registry.flush_pending_metrics().await.unwrap();
            assert!(parsing_metric_points(&registry).is_empty());
            let points = parser_metric_points(&registry);
            assert_eq!(
                parser_metric_value(
                    &points,
                    "operations",
