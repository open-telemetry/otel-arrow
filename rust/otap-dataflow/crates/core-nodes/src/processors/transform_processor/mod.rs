...
pub struct TransformProcessor {
    ...
    log_parser: Option<parse_logs::Parser>,
    contexts: Contexts,
...
impl TransformProcessor {
    ...
    fn from_config(pipeline_ctx: &PipelineContext, config: &Value) -> Result<Self, ConfigError> {
        if ["kql_query", "opl_query", "ottl", "parse_logs"]
            .iter()
        ...
        let (transforms, language) = match config.query {
            ...
            Query::ParseLogs(parse_config) => {
                log_parser = Some(
                    parse_logs::Parser::new(*parse_config)
                        .map_err(|error| ConfigError::InvalidUserConfig { error })?,
                );
                (Vec::new(), TransformLanguage::ParseLogs)
            }
        };
...
fn validate_transform_config(config: &Value) -> Result<(), ConfigError> {
    ...
    if let Query::ParseLogs(config) = parsed.query {
        let _ = parse_logs::Parser::new(*config)
            .map_err(|error| ConfigError::InvalidUserConfig { error })?;
    }
    Ok(())
}
        ...
        match message {
            ...
            Message::PData(pdata) => {
                ...
                if let Some(parser) = &self.log_parser
                    && pdata_signal_type == SignalType::Logs
                {
                    transformed = true;
                    let conversion: Result<OtapArrowRecords, _> = payload
                        .take()
                    ...
                    match conversion {
                        ...
                        Ok(mut batch) => match batch.decode_transport_optimized_ids() {
                            Err(error) => {
                                transform_error = Some(TransformOperationError::new(
                            ...
                            Ok(()) => match parser.apply(batch) {
                                Ok((batch, counts)) => {
                                    self.metrics.record_parsing(parser.format(), counts);
                                    payload = Some(batch.into());
                                }
                                Err(error) => {
                                    transform_error = Some(TransformOperationError::new(
                                        TransformErrorType::Internal,
...
mod test {
    ...
    fn test_parse_logs_framed_batches() {
        for (format, input, body, text, number) in [
            ...
            let node = include_str!("README.md")
                ...
                .find(|node| node["config"]["parse_logs"]["format"] == format)
                .unwrap();
    ...
    /// Guarantees: Invalid modes fail at construction while existing query configuration remains valid.
    #[test]
    fn test_parse_logs_configuration_gate() {
        let runtime = TestRuntime::<OtapPdata>::new();
        assert!(try_create_with_config(json!({"opl_query":"logs | where true"}), &runtime).is_ok());
        let mut config = parsing_config("json");
        config["opl_query"] = "logs".into();
        assert!(try_create_with_config(config, &runtime).is_err());
        let mut config = parsing_config("json");
        config["parse_logs"]["unknown"] = true.into();
        assert!(try_create_with_config(config, &runtime).is_err());
        let mut config = parsing_config("regex");
        config["parse_logs"]["limits"]["max_pattern_bytes"] = 1.into();
        assert!((TRANSFORM_PROCESSOR_FACTORY.validate_config)(&config).is_err());
        assert!(try_create_with_config(config, &runtime).is_err());
    }
    ...
    fn test_documented_parsing_yaml() {
        ...
        for block in documentation.split("```yaml").skip(1) {
            ...
            if node["config"].get("parse_logs").is_some() {
                (TRANSFORM_PROCESSOR_FACTORY.validate_config)(&node["config"]).unwrap();
    ...
    fn test_parse_logs_fallback_records_and_counters() {
        for (timestamp, event, observed, policy, severity, expected_time, reason) in [
            ...
            let mut config = parsing_config("json");
            config["parse_logs"]["timestamp"]["on_missing"] = policy.into();
            ...
            runtime
                ...
                .validate(move |_ctx| async move {
                    ...
                    let points = metric_points_for(&registry, "processor.transform.parse_logs");
                    match reason {
                        Some(reason) => {
                            assert_eq!(
                                points.len(),
    ...
    fn assert_parsing_fixture(
        ...
        let registry = runtime.metrics_registry();
        let reporter = runtime.metrics_reporter();
        let format = config["parse_logs"]["format"].as_str().unwrap().to_owned();
        let processor = try_create_with_config(config, &runtime).unwrap();
    ...
    fn test_parse_logs_shape_and_fragment_records() {
        ...
        for (pointer, input) in [
            ...
            let mut config = parsing_config("json");
            config["parse_logs"]["body"]["source"] = pointer.into();
            assert_parsing_fixture(
                config,
                vec![AnyValue::new_string(input)],
                Some("body"),
    ...
    fn test_parse_logs_limit_records() {
        for (format, input, body) in [
            ...
            let parser = parse_logs::Parser::new(
                serde_json::from_value(config["parse_logs"].clone()).unwrap(),
            )
            .unwrap();
            for (limit, bound) in [
                ...
                for over in [false, true] {
                    ...
                    config["parse_logs"]["limits"][limit] = (bound - usize::from(over)).into();
                    assert_parsing_fixture(
                        config,
                        vec![AnyValue::new_string(input)],
                        (!over).then_some(body),
        ...
        for (limit, bound) in [("max_json_depth", 2), ("max_entries", 4)] {
            for over in [false, true] {
                let mut config = parsing_config("json");
                config["parse_logs"]["body"]["source"] = "/nested/body".into();
                config["parse_logs"]["limits"][limit] = (bound - usize::from(over)).into();
                assert_parsing_fixture(
                    config,
                    vec![AnyValue::new_string(input)],
        ...
        config["parse_logs"]["limits"]["max_entries"] = 3.into();
        assert_parsing_fixture(
            config.clone(),
    ...
    fn test_parse_logs_non_log_passthrough() {
        ...
        runtime
            ...
            .validate(move |_ctx| async move {
                registry.flush_pending_metrics().await.unwrap();
                assert!(metric_points_for(&registry, "processor.transform.parse_logs").is_empty());
    ...
    fn test_parse_logs_factory_rejects_invalid_settings() {
        for (format, pointer, value) in [
            ("regex", "/parse_logs/severity/source", json!("absent")),
            ("regex", "/parse_logs/pattern", json!("(")),
            (
                "regex",
                "/parse_logs/limits/max_compiled_regex_bytes",
                json!(1),
            ),
            ("csv", "/parse_logs/columns", json!(["ts", "sev", "sev"])),
            ("csv", "/parse_logs/body/source", json!("absent")),
            ("csv", "/parse_logs/header", json!("infer")),
            ("csv", "/parse_logs/delimiter", json!("||")),
            ("json", "/parse_logs/body/source", json!("/bad~2")),
            ("json", "/parse_logs/severity/mapping/ERROR", json!(0)),
            ("json", "/parse_logs/severity/mapping/ERROR", json!(25)),
            ("json", "/parse_logs/limits/max_input_bytes", json!(0)),
            ("json", "/parse_logs/limits/max_scratch_bytes", json!(0)),
            ("json", "/parse_logs/limits/max_json_depth", json!(0)),
            ("json", "/parse_logs/limits/max_entries", json!(0)),
        ] {
            let mut config = parsing_config(format);
            *config.pointer_mut(pointer).unwrap() = value;
            assert!(
                (TRANSFORM_PROCESSOR_FACTORY.validate_config)(&config).is_err(),
                "{pointer}"
            );
        }
    }
    ...
    fn test_parse_logs_failed_send() {
        ...
        let mut node = NodeUserConfig::new_processor_config(TRANSFORM_PROCESSOR_URN);
        node.config = parsing_config("json");
        node.default_output = Some("closed".into());
        let mut processor = create_transform_processor(
            pipeline,
            test_node("transform-processor"),
            Arc::new(node),
            runtime.config(),
            &otel_arrow_dfe_engine::capability::registry::Capabilities::empty(),
        )
    ...
    fn test_parse_logs_internal_failure_after_staging() {
        ...
        let mut processor = TransformProcessor::from_config(&pipeline, &node.config).unwrap();
        processor.log_parser.as_mut().unwrap().fail_after = Some(1);
