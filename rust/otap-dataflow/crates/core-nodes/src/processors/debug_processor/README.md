...
universal `node.consumed` metrics instead of being duplicated by this
processor. Use `node.consumed.messages` for requests and enable item counts to
use `node.consumed.items`.
