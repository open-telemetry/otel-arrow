...
use crate::stress::{calculate_disk_usage, SteadyStateStats, StressStats};
...
impl Dashboard {
    ...
    pub fn update(
        ...
        let bundles_this_period = stats.total_bundles_ingested.saturating_sub(self.last_iteration_bundles);
    ...
    pub fn update_steady_state(
        ...
        let now = Instant::now();
        let iteration_elapsed = now.duration_since(self.last_iteration_time);
        let bundles_this_period = stats.total_ingested.saturating_sub(self.last_iteration_bundles);
        ...
        if iteration_elapsed >= Duration::from_secs(1) {
            ...
            // Memory history (store as MB * 10 for better sparkline resolution)
            self.memory_history.push((stats.current_memory_mb * 10.0) as u64);
            ...
            let seg_rate_kbs = (seg_bytes_this_period as f64
            ...
            let wal_rate_kbs = (wal_bytes_this_period as f64
                / iteration_elapsed.as_secs_f64()
                / 1024.0) as u64;
...
fn render_header(frame: &mut Frame<'_>, area: Rect, elapsed: Duration, target: Duration, progress_pct: f64) {
...
fn render_stats(
    ...
    let iterations_text = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{}", stats.iterations), Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
        ]),
        Line::from(vec![
            Span::styled("Ingested:   ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", stats.total_bundles_ingested), Style::default().fg(Color::Green)),
        ]),
        Line::from(vec![
            Span::styled("Consumed:   ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", stats.total_bundles_consumed), Style::default().fg(Color::Blue)),
        ]),
        Line::from(vec![
            Span::styled("Throughput: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.0}/s", avg_throughput), Style::default().fg(Color::Yellow)),
        ]),
    ];
    ...
    let memory_text = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:.1} MB", current_memory), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Initial:  ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", stats.initial_memory_mb), Style::default().fg(Color::Gray)),
        ]),
        Line::from(vec![
            Span::styled("Growth:   ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:+.1} MB", memory_growth), Style::default().fg(memory_color)),
        ]),
        Line::from(vec![
            Span::styled("Peak:     ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", stats.peak_memory_mb), Style::default().fg(Color::Magenta)),
        ]),
    ];
    ...
    let first_disk = stats.disk_history.first().map(|(_, b)| *b).unwrap_or(0) as f64 / 1024.0 / 1024.0;
    let disk_growth = disk_mb - first_disk;

    let disk_text = vec![
        Line::from(vec![
            Span::styled("Current:  ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", disk_mb), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Initial:  ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", first_disk), Style::default().fg(Color::Gray)),
        ]),
        Line::from(vec![
            Span::styled("Growth:   ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:+.1} MB", disk_growth), Style::default().fg(if disk_growth > 100.0 { Color::Red } else { Color::Green })),
        ]),
        Line::from(""),
    ];
...
fn render_throughput_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    let sparkline = Sparkline::default()
        .block(Block::default().title(format!(" Throughput (bundles/sec) │ max: {} ", max_val)).borders(Borders::ALL))
        .data(history)
        .max(max_val)
...
fn render_memory_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64], current: f64) {
    ...
    let sparkline = Sparkline::default()
        .block(Block::default().title(format!(" Memory (MB) │ current: {:.1} ", current)).borders(Borders::ALL))
        .data(history)
        .max(max_val)
...
fn render_cpu_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    let sparkline = Sparkline::default()
        .block(Block::default().title(format!(" CPU │ current: {:.1}% ", current)).borders(Borders::ALL))
        .data(history)
        .max(max_val)
...
fn render_disk_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    let sparkline = Sparkline::default()
        .block(Block::default().title(format!(" Disk │ current: {} MB ", current)).borders(Borders::ALL))
        .data(history)
        .max(max_val)
...
fn render_segment_write_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    let title = if current >= 1024 {
        format!(" Segment Writes (.qseg) │ current: {:.1} MB/s ", current as f64 / 1024.0)
    } else {
...
fn render_wal_write_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    // Display in MB/s if >= 1024 KB/s, otherwise KB/s
    let title = if current >= 1024 {
        format!(" WAL Writes (quiver.wal*) │ current: {:.1} MB/s ", current as f64 / 1024.0)
    } else {
...
fn render_backlog_sparkline(frame: &mut Frame<'_>, area: Rect, history: &[u64]) {
    ...
    let sparkline = Sparkline::default()
        .block(Block::default().title(format!(" Backlog │ current: {} bundles ", current)).borders(Borders::ALL))
        .data(history)
        .max(max_val)
...
fn render_steady_state_stats(
    ...
    let bundles_text = vec![
        ...
        Line::from(vec![
            Span::styled("Consumed: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", stats.total_consumed), Style::default().fg(Color::Blue)),
        ]),
        Line::from(vec![
            Span::styled("Backlog:  ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{}", stats.backlog),
                Style::default().fg(if stats.backlog > 1000 { Color::Yellow } else { Color::White }),
            ),
        ]),
    ];
    ...
    let segments_text = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{}", stats.total_cleaned), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::styled("Active:   ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{}", stats.total_segments_written.saturating_sub(stats.total_cleaned)),
    ...
    let rates_text = vec![
        Line::from(vec![
            ...
            Span::styled(format!("{:.0}/s", ingest_rate), Style::default().fg(Color::Green)),
        ]),
        Line::from(vec![
            Span::styled("Consume: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.0}/s", consume_rate), Style::default().fg(Color::Blue)),
        ]),
        Line::from(vec![
            Span::styled("Rows:    ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.0}/s", stats.consume_rows_rate()), Style::default().fg(Color::Cyan)),
        ]),
        Line::from(vec![
            Span::styled("Data:    ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB/s", stats.consume_mb_rate()), Style::default().fg(Color::Magenta)),
        ]),
    ];

    let rates_block = Paragraph::new(rates_text)
        .block(Block::default().title(" Rates ").borders(Borders::ALL));
    ...
    let memory_text = vec![
        Line::from(vec![
            Span::styled("Current: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", stats.current_memory_mb), Style::default().fg(Color::White)),
        ]),
        Line::from(vec![
            Span::styled("Initial: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", stats.initial_memory_mb), Style::default().fg(Color::Gray)),
    ...
    let disk_text = vec![
        ...
        Line::from(vec![
            Span::styled("Initial: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", initial_disk_mb), Style::default().fg(Color::Gray)),
        ]),
        Line::from(vec![
            Span::styled("Growth:  ", Style::default().fg(Color::Gray)),
            Span::styled(
                format!("{:+.1} MB", disk_growth),
                Style::default().fg(if disk_growth > 100.0 { Color::Red } else { Color::Green }),
            ),
        ]),
        Line::from(vec![
            Span::styled("Peak:    ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{:.1} MB", peak_disk_mb), Style::default().fg(Color::Magenta)),
        ]),
    ];
