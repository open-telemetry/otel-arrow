...
mod tests {
    ...
    fn pipeline_summary() -> ErrorSummary {
        ErrorSummary::Pipeline {
            error_kind: "runtime".into(),
            message: "pipeline failed".into(),
            source: Some("root cause".into()),
        }
    }

    /// Scenario: every error event variant is normalized for structured logging.
    /// Guarantees: event kinds are stable and only deadline events omit a summary.
    #[test]
    fn error_event_accessors_cover_all_variants() {
        let cases = [
            (
                ErrorEvent::AdmissionError(pipeline_summary()),
                "admission_error",
                true,
            ),
            (
                ErrorEvent::ConfigRejected(pipeline_summary()),
                "config_rejected",
                true,
            ),
            (
                ErrorEvent::UpdateFailed(pipeline_summary()),
                "update_failed",
                true,
            ),
            (
                ErrorEvent::RollbackFailed(pipeline_summary()),
                "rollback_failed",
                true,
            ),
            (
                ErrorEvent::DrainError(pipeline_summary()),
                "drain_error",
                true,
            ),
            (
                ErrorEvent::DrainDeadlineReached,
                "drain_deadline_reached",
                false,
            ),
            (
                ErrorEvent::RuntimeError(pipeline_summary()),
                "runtime_error",
                true,
            ),
            (
                ErrorEvent::DeleteError(pipeline_summary()),
                "delete_error",
                true,
            ),
        ];

        for (event, expected_kind, has_summary) in cases {
            assert_eq!(event.kind(), expected_kind);
            assert_eq!(event.summary().is_some(), has_summary);
        }
    }

    /// Scenario: pipeline-level and node-level summaries expose compact log fields.
    /// Guarantees: common diagnostics and optional node identity retain their values.
    #[test]
    fn error_summary_accessors_preserve_fields() {
        let pipeline = pipeline_summary();
        assert_eq!(pipeline.error_kind(), "runtime");
        assert_eq!(pipeline.message(), "pipeline failed");
        assert_eq!(pipeline.source(), Some("root cause"));
        assert_eq!(pipeline.node(), None);

        let node = ErrorSummary::Node {
            node: "debug".into(),
        ...
        assert_eq!(node.error_kind(), "transport");
        assert_eq!(node.message(), "write failed");
        assert_eq!(node.source(), None);
        assert_eq!(node.node(), Some(("debug", &NodeKind::Processor)));
    }
}
