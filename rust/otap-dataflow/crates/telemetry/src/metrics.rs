...
use crate::attributes::{AttributeSetHandler, DynamicAttributeSet};
use crate::descriptor::{
    DynamicAttributeDescriptor, Instrument, MetricsDescriptor, MetricsField, Temporality,
};
...
/// dynamic metric set's worst-case cardinality (the product of its enum
...
///
/// Generated code for a dynamic metric set evaluates
/// `check_cardinality(<D as DynamicAttributeSet>::CARDINALITY)` in a `const`
...
pub const fn check_cardinality(cardinality: usize) {
    assert!(
        ...
        "metric set worst-case cardinality exceeds CARDINALITY_BUDGET; \
         reduce the number of dynamic enum attributes or their variants"
    );
}
...
pub struct MetricSetSnapshot {
    ...
    /// only static attributes; for dynamic sets it selects the datapoint whose
    /// enum-attribute combination decodes from this index (see
    /// [`DynamicAttributeSet::bucket_index`]).
...
impl MetricSetSnapshot {
    ...
    /// Returns the bucket index this snapshot targets (0 for non-dynamic sets).
    #[must_use]
...
/// A [`MetricSetHandler`] that binds a set of dynamic (per-datapoint) enum
/// attributes, generated by `#[metric_set(dynamic_attributes = ...)]`.
///
/// The associated [`DynamicAttributes`](Self::DynamicAttributes) type identifies
/// the [`DynamicAttributeSet`] whose variants address the set's buckets.
pub trait DynamicMetricSetHandler: MetricSetHandler + Default {
    ...
    type DynamicAttributes: DynamicAttributeSet;
}

/// A [`MetricSetHandler`] that binds a set of static (fixed-at-registration)
/// attributes, generated by `#[metric_set(static_attributes = ...)]`.
pub trait StaticMetricSetHandler: MetricSetHandler + Default {
    /// The attribute set supplied once at registration and attached to every
    /// datapoint of this set.
    type StaticAttributes: AttributeSetHandler;
}
...
pub trait MetricSetRegistrar {
    ...
    /// Registers a metric set with bounded per-datapoint attributes.
    fn register_dynamic_metric_set<M: DynamicMetricSetHandler + Debug + Send + Sync>(
        &self,
    ) -> DynamicMetricSet<M>;

    /// Registers a metric set with fixed and per-datapoint attributes.
...
/// A registered dynamic metric set: a dense array of per-bucket metric structs
/// addressed by a [`DynamicAttributeSet`]'s mixed-radix bucket index.
///
...
/// been written so only live datapoints are reported.
pub struct DynamicMetricSet<M: DynamicMetricSetHandler> {
    pub(crate) key: MetricSetKey,
    pub(crate) entity_key: EntityKey,
...
impl<M: DynamicMetricSetHandler + Debug> Debug for DynamicMetricSet<M> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("DynamicMetricSet")
            .field("key", &self.key)
...
impl<M: DynamicMetricSetHandler> DynamicMetricSet<M> {
    pub(crate) fn new(key: MetricSetKey, entity_key: EntityKey) -> Self {
        let mut buckets = Vec::with_capacity(M::DynamicAttributes::CARDINALITY);
        buckets.resize_with(M::DynamicAttributes::CARDINALITY, M::default);
        Self {
            key,
            entity_key,
            buckets,
            touched: vec![0u64; M::DynamicAttributes::CARDINALITY.div_ceil(64)],
        }
    }
    ...
    #[inline]
    pub fn with(&mut self, attrs: M::DynamicAttributes) -> &mut M {
    ...
    /// Returns the metric set key associated with this dynamic metric set.
    #[must_use]
    pub const fn metric_set_key(&self) -> MetricSetKey {
        self.key
    }

    /// Returns the entity key associated with this dynamic metric set.
    #[must_use]
    pub const fn entity_key(&self) -> EntityKey {
        self.entity_key
    }
...
pub struct MetricsEntry {
    ...
    /// Number of datapoint buckets (1 for plain and static-only sets, the
    /// [`DynamicAttributeSet::CARDINALITY`] for dynamic sets).
    pub bucket_count: usize,

    /// Per-datapoint enum attribute descriptors used to decode a bucket index
    /// into datapoint attributes at export time (empty for non-dynamic sets).
    pub dynamic_attributes: &'static [DynamicAttributeDescriptor],

    /// Fixed (key, value) attributes attached to every datapoint of this set,
    /// captured at registration (empty for sets without static attributes).
...
impl MetricsEntry {
    /// Creates a new plain metrics entry (single bucket, no per-datapoint attributes).
    #[must_use]
    pub const fn new(
    ...
    /// Creates a metrics entry with fixed attributes and `bucket_count` dynamic
    /// buckets. The value vector is pre-sized to `bucket_count * fields` zeroed
    /// slots.
    #[must_use]
    pub fn new_with_datapoint_attributes(
        metrics_descriptor: &'static MetricsDescriptor,
        zeroed_bucket: &[MetricValue],
        entity_key: EntityKey,
        bucket_count: usize,
        dynamic_attributes: &'static [DynamicAttributeDescriptor],
...
impl MetricSetRegistry {
    ...
    >(
        ...
        let metrics_key = self
            .metrics
            .insert(MetricsEntry::new_with_datapoint_attributes(
                descriptor,
                &metrics.snapshot_values(),
                entity_key,
                1,
    ...
    pub(crate) fn register_with_dynamic_attributes<M>(
        &mut self,
        entity_key: EntityKey,
    ) -> DynamicMetricSet<M>
    where
        M: DynamicMetricSetHandler + Debug + Send + Sync,
    {
        ...
        let metrics_key = self
            ...
            .insert(MetricsEntry::new_with_datapoint_attributes(
                ...
                &zeroed_bucket,
                entity_key,
                M::DynamicAttributes::CARDINALITY,
                M::DynamicAttributes::DESCRIPTORS,
                Vec::new(),
            ));
    ...
    /// Registers a metric set with fixed attributes and one bucket per dynamic
    /// attribute combination.
    pub(crate) fn register_with_static_and_dynamic_attributes<M>(
        &mut self,
        entity_key: EntityKey,
        static_attributes: Vec<(String, String)>,
    ) -> DynamicMetricSet<M>
    ...
    /// Visits every non-empty datapoint bucket of every metric set, yielding the
    /// per-datapoint enum/static attributes alongside a zero-alloc iterator of
    /// `(MetricsField, value)`, then resets the visited bucket to zero.
    pub(crate) fn visit_and_reset_with_datapoint_attrs<F>(
        &mut self,
    ...
    /// Read-only variant of [`Self::visit_and_reset_with_datapoint_attrs`] that
    /// does not reset bucket values.
    pub(crate) fn visit_current_with_datapoint_attrs<F>(
        &self,
        entities: &EntityRegistry,
        mut f: F,
        keep_all_zeroes: bool,
...
/// Decodes a dense mixed-radix `bucket` index into datapoint attributes.
///
/// Static attributes are emitted first (fixed key/value pairs), followed by the
/// dynamic enum attributes. For the dynamic axis the first declared attribute is
/// the low-order digit: `variant_index = (rem % radix); rem /= radix`.
fn decode_bucket_datapoint_attrs<'a>(
    dynamic: &'a [DynamicAttributeDescriptor],
    static_attrs: &'a [(String, String)],
    bucket: usize,
...
mod tests {
    ...
    fn test_accumulate_snapshot_basic() {
        ...
        metrics.visit_and_reset_with_datapoint_attrs(
            &entities,
