...
pub trait StaticMetricSetHandler: MetricSetHandler + Default {
    ...
    type Attributes: AttributeSetHandler;
}
...
pub struct DynamicMetricSet<M: MetricSetHandler, D: DynamicAttributeSet> {
    pub(crate) key: MetricSetKey,
    pub(crate) entity_key: EntityKey,
...
impl<M: MetricSetHandler + Debug, D: DynamicAttributeSet> Debug for DynamicMetricSet<M, D> {
...
impl<M: MetricSetHandler + Default, D: DynamicAttributeSet> DynamicMetricSet<M, D> {
    pub(crate) fn new(key: MetricSetKey, entity_key: EntityKey) -> Self {
        let mut buckets = Vec::with_capacity(D::CARDINALITY);
        buckets.resize_with(D::CARDINALITY, M::default);
        Self {
            key,
            entity_key,
            buckets,
            touched: vec![0u64; D::CARDINALITY.div_ceil(64)],
            _pd: PhantomData,
        }
    }
    ...
    #[inline]
    pub fn with(&mut self, attrs: D) -> &mut M {
...
impl MetricsEntry {
    ...
    /// Creates a dynamic metrics entry with `bucket_count` buckets and the given
