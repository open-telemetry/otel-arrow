...
pub struct MetricSetExport {
    ...
    /// String attributes attached to every OTLP data point in this bucket.
    pub datapoint_attributes: Vec<(String, String)>,
...
pub(crate) struct MetricExportCheckpoint {
    ...
    /// Identifies the datapoint bucket represented by the batch entry.
    bucket: usize,
    /// Restores the original delta window if delivery is rolled back.
    delta_start_time_unix_nano: u64,
}

/// A [`MetricSetHandler`] that binds a set of measurement (per-datapoint) enum
...
pub trait MeasurementMetricSetHandler: MetricSetHandler + Default {
    /// The measurement attribute set whose combinations index this set's datapoints.
...
pub trait MetricSetRegistrar {
    ...
    /// Registers a metric set with registration-time datapoint attributes.
...
pub struct MetricsEntry {
    ...
    /// Number of datapoint buckets (1 for plain and registration-only sets, the
    ...
    /// into datapoint attributes at export time (empty for non-measurement sets).
...
impl MetricSetRegistry {
    ...
    /// Registers a metric set carrying registration-time datapoint attributes captured
    ...
    /// per-datapoint enum/registration attributes alongside a zero-alloc iterator of
        ...
        ),
    {
        ...
        for (metrics_key, entry) in &mut self.metrics {
            ...
            let mut datapoint_attributes = Vec::new();
        ...
        for (metrics_key, entry) in &mut self.metrics {
            ...
            for bucket in 0..entry.bucket_count {
                    ...
                    && let Some(attributes) = attributes.clone()
                {
                    ...
                    let datapoint_attributes = decoded_attributes
                        .iter()
...
mod tests {
    ...
    fn test_bucketed_export_rollback_restores_each_bucket_independently() {
        ...
        assert_eq!(
            batch.metric_sets[0].datapoint_attributes,
            vec![("outcome".to_owned(), "accepted".to_owned())]
        );
        assert_eq!(
            batch.metric_sets[1].datapoint_attributes,
