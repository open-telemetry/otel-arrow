...
pub struct Generator {
    ...
    /// them. Values are optional fixed strings; when left empty (`None`), a
    /// random value is generated once at startup by the traffic generator.
    pub(crate) tenant_values: HashMap<String, Option<String>>,
...
impl Generator {
    ...
    /// Configure tenant values to inject into generated traffic.
    ///
    /// Each key names a tenant token key as an inbound request would have
    /// supplied it; the value is an optional fixed string. When the value is
    /// `None`, the traffic generator assigns a random value at startup.
    #[must_use]
    pub fn with_tenant_values(
        mut self,
        values: impl IntoIterator<Item = (impl Into<String>, Option<impl Into<String>>)>,
    ) -> Self {
        self.tenant_values = values
            .into_iter()
...
mod tests {
    ...
    fn generator_defaults_match_expected() {
        ...
        assert!(g.tenant_values.is_empty());
    }
    ...
    #[test]
    fn generator_with_tenant_values() {
        let g = Generator::logs().with_tenant_values([
            ("x-tenant-id", Some("acme")),
            ("x-request-id", None::<&str>),
        ]);
        assert_eq!(g.tenant_values.len(), 2);
        assert_eq!(
            g.tenant_values.get("x-tenant-id"),
            Some(&Some("acme".to_string()))
        );
        assert_eq!(g.tenant_values.get("x-request-id"), Some(&None::<String>));
    }
}
