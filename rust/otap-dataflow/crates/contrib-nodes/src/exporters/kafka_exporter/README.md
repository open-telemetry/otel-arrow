...
The configured `topic_from_transport_header` spelling is preserved. The router
matches it against stored transport-header names using ASCII case-insensitive
comparison. This works both for lowercase names generated by capture rules
without `store_as` and for explicitly configured mixed-case `store_as` names.
If multiple stored names differ only by ASCII case, the first header wins.

The selected header's value is used as the Kafka topic exactly as supplied; the
exporter does not lowercase topic names. Topic validation and configured
allowlists are applied to that exact value.
