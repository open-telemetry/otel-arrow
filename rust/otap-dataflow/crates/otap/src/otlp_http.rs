        ...
        let decoded = read_to_end_limited(&mut decoder, max_len).map_err(|e| {
            ...
            if err_msg.contains("decoded body too large") {
                ...
                otap_df_telemetry::otel_debug!(
                    "otlp.receiver.http.request_rejected",
...
impl HttpHandler {
    async fn handle(self, req: Request<Incoming>) -> Result<Response<Full<Bytes>>, Infallible> {
        ...
        let fut = async move {
                ...
                match permit_result {
                    ...
                    Ok(Err(_)) => {
                        ...
                        otap_df_telemetry::otel_error!(
                            "otlp.receiver.http.semaphore_closed",
                            kind = "global",
            ...
            {
                otap_df_telemetry::otel_warn!(
                    "otlp.receiver.http.pipeline_send_failed",
                    path = parts.uri.path().to_string(),
            ...
            if let Some((_guard, rx)) = cancel_rx {
                match rx.await {
                    ...
                    Ok(Err(nack)) => {
                        otap_df_telemetry::otel_debug!(
                            "otlp.receiver.http.request_nacked",
        ...
        let result = if let Some(timeout_duration) = timeout {
            match tokio::time::timeout(timeout_duration, fut).await {
                ...
                Err(_) => {
                    otap_df_telemetry::otel_warn!(
                        "otlp.receiver.http.request_timeout",
                        path = path.as_str(),
...
pub async fn serve(
        ...
        tokio::select! {
            ...
            accept_result = listener.accept() => {
                ...
                let stream = match socket_options::apply_socket_options(
                    ...
                    Err(e) => {
                        otap_df_telemetry::otel_warn!(
                            "otlp.receiver.http.socket_options_failed",
                    ...
                    if let Some(acceptor) = maybe_tls_acceptor.clone() {
                        ...
                        drop(tracker.spawn(async move {
                            ...
                            tokio::select! {
                                res = &mut conn => {
                                    if let Err(err) = res {
                                        otap_df_telemetry::otel_debug!("otlp.receiver.http.connection_error", error = err.to_string());
                                    }
                                },
                                _ = shutdown.cancelled() => {
    ...
    {
        otap_df_telemetry::otel_warn!(
            "otlp.receiver.http.shutdown_timeout",
