  ...
  syslog_cef:
    protocol:
      tcp:
        listening_addr: "0.0.0.0:514"
    batch:
      flush_interval_ms: 50    # Flush after 50 ms (default: 100)
...
| `batch.flush_interval_ms` | `integer` | No | Max ms before flushing a batch (default: `100`) |
...
- **`batch.flush_interval_ms`** milliseconds have elapsed since the last flush
  (default: 100 ms), or
