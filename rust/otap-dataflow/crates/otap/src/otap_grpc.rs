    ...
    F: Fn(T) -> OtapArrowRecords,
{
    ...
    let batch = consumer.consume_bar(&mut batch).map_err(|e| {
        otel_error!("otap.batch.decode_failed", "Error decoding OTAP Batch. Closing stream", error = ?e);
    })?;
    ...
    let cancel_rx = if let Some(state) = state {
        ...
        let allocation_result = {
            ...
            match guard_result {
                ...
                Err(_) => {
                    otel_error!("otap.mutex.poisoned", "Mutex poisoned");
                    return Err(());
                }
            }
        ...
        let (key, rx) = match allocation_result {
            None => {
                ...
                .await
                .map_err(|e| {
                    otel_error!("otap.response.send_failed", "Error sending BatchStatus response", error = ?e);
                })?;
        ...
        .await
    {
        Ok(_) => {}
        Err(e) => {
            otel_error!("otap.pipeline.send_failed", "Failed to send to pipeline", error = ?e);
            return Err(());
        }
    };
