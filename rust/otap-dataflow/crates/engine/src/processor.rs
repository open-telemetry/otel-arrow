...
mod tests {
    ...
    /// Scenario: A decision-only flow node records dropped signals without acting as a flow start or end.
    /// Guarantees: Dropped metrics are reported and incoming/outgoing counters remain no-ops for the node.
    #[test]
    ...
    /// Guarantees: Flow incoming, duration, and outgoing metrics are emitted automatically.
