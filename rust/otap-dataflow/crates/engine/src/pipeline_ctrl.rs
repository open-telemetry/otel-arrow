...
impl<PData> PipelineCtrlMsgManager<PData> {
    ...
    pub async fn run(mut self) -> Result<(), Error> {
        loop {
            ...
            let next_earliest = opt_min(
                opt_min(next_expiry, next_tel_expiry),
                next_delay_expiry.map(|x| x.into()),
            );
            ...
            tokio::select! {
                ...
                PipelineControlMsg::DelayData(delayed) => {
                    ...
                    _ = async {
                        ...
                        while self.delayed_data.peek().map(|d| d.when <= now.into_std()).unwrap_or(false) {
                let delayed = self.delayed_data.pop().expect("ok");
                to_send.push((delayed.node_id, NodeControlMsg::DelayedData {
                when: delayed.when,
                data: delayed.data,
            }));
                        }
...
mod tests {
    ...
    async fn test_delayed_data_heap_ordering() {
        ...
        assert_eq!(
            first.when,
            (now + Duration::from_millis(100)).into(),
        ...
        assert_eq!(
            second.when,
            (now + Duration::from_millis(200)).into(),
        ...
        assert_eq!(
            third.when,
            (now + Duration::from_millis(300)).into(),
