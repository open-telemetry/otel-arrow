...
pub struct PipelineCtrlMsgManager<PData> {
    ...
    /// which would cause a circular-wait deadlock on the single-threaded
    /// LocalSet runtime.
...
impl<PData> PipelineCtrlMsgManager<PData> {
    ...
    pub async fn run(mut self) -> Result<(), Error> {
        ...
        loop {
            ...
            tokio::select! {
                ...
                // Retry pending sends after a short delay so the loop
                // wakes up even when no other events arrive.
    ...
    /// loop and caused a circular-wait deadlock (fanout ↔ exporter ↔ ctrl manager).
    ///
        ...
        if let Some(sender) = self.control_senders.get(node_id) {
            match sender.try_send(msg) {
                ...
                Err(otap_df_channel::error::SendError::Full(msg)) => {
                    // Buffer instead of blocking — breaks the deadlock cycle
    ...
    /// delivered are re-queued at the back.
    fn drain_pending_sends(&mut self) {
        let mut remaining = VecDeque::new();
        while let Some((node_id, msg)) = self.pending_sends.pop_front() {
            if let Some(sender) = self.control_senders.get(node_id) {
                match sender.try_send(msg) {
                    Ok(()) => {}
                    Err(otap_df_channel::error::SendError::Full(msg)) => {
                        remaining.push_back((node_id, msg));
                    }
                    Err(otap_df_channel::error::SendError::Closed(_)) => {
