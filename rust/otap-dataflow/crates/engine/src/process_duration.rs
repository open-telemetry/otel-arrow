...
//! per-message flow_measurement. [`ComputeDuration::timed`] provides the
...
//! total wall-clock compute between sends for flow_measurements without processor
//! cooperation.
//!
...
impl ComputeDuration {
    ...
    /// the `processor.compute.{success,failed}.duration` metric; flow_measurement
