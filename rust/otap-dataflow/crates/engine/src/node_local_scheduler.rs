    ...
    fn pop_due(&mut self, now: Instant) -> Option<NodeControlMsg<PData>> {
        ...
        if take_resume {
            ...
            return Some(NodeControlMsg::ResumeData {
                when: resume.when,
                data: resume.data,
                resume_id: LocalResumeId::new(resume.sequence),
            });
        }
...
mod tests {
    ...
    fn expect_resume(
        ...
        match msg {
            Some(NodeControlMsg::ResumeData {
                when,
                data,
                resume_id,
            }) => {
                ...
                assert_eq!(resume_id, expected_resume_id);
            }
    ...
    /// `NodeControlMsg::ResumeData` with the returned resume id and clears the
    ...
    fn requeue_later_emits_the_stored_payload() {
        ...
        expect_resume(scheduler.pop_due(when), when, 17, resume_id);
    ...
    fn delayed_resumes_preserve_due_time_ordering() {
        ...
        expect_resume(scheduler.pop_due(sooner), sooner, 0, sooner_id);
        expect_resume(
        ...
        expect_resume(scheduler.pop_due(later), later, 3, later_id);
    }
    ...
    /// `ResumeData` delivery using the shutdown-start timestamp while
    ...
    fn shutdown_makes_pending_delayed_resumes_due_immediately() {
        ...
        expect_resume(scheduler.pop_due(now), now, 11, first_id);
        expect_resume(scheduler.pop_due(now), now, 12, second_id);
