...
impl ExtensionLifecycle {
    /// Spawn all active+background extensions onto `local_runtime` and
    ...
    pub fn spawn(
        ...
        for ext_wrapper in extensions {
            ...
            futures.push(local_runtime.spawn_local(fut));
        }
...
mod tests {
    ...
    fn drain_until_deadline_is_bounded_for_stuck_extension() {
        ...
        rt.block_on(async {
            let futures = FuturesUnordered::new();
            futures.push(rt.spawn_local(async {
                // Misbehaving extension that ignores `Shutdown` and
                // never returns from `start()`. `pending` is cancelled
                // when the surrounding runtime drops at the end of the
                // test, so this does not actually run forever.
