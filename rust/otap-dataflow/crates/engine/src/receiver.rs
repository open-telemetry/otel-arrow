...
impl<PData> Node for ReceiverWrapper<PData> {
    ...
    fn user_config(&self) -> Rc<NodeUserConfig> {
        match self {
            ReceiverWrapper::Local { user_config: config, .. } => config.clone(),
            ReceiverWrapper::Shared { user_config: config, .. } => config.clone(),
        }
    }
