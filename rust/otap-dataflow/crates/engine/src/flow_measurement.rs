...
//! Flow measurements that sum per-message compute duration and signal counts
//! across a contiguous range of processor nodes.
//!
//! Flow measurements are declared in the pipeline telemetry policy YAML and
...
use otap_df_config::policy::{FlowMeasurement, TelemetryPolicy};
...
/// Index of a flow_measurement within the pipeline's flow_measurement table.
pub type FlowMeasurementId = usize;

/// Per-pipeline flow_measurement state.
///
...
pub(crate) struct PipelineFlowMeasurementState {
...
/// Start-side measurements for a node that begins a flow_measurement range.
///
/// Groups the metric set and its accumulator so that they share the
/// `Option` on `FlowMeasurementState` — non-start nodes pay no allocation for
/// either.
#[derive(Clone)]
pub(crate) struct IncomingMeasurements<Accumulator> {
...
/// Stop-side measurements for a node that terminates a flow_measurement range.
///
/// Groups the metric set and its accumulators so that they share the
/// `Option` on `FlowMeasurementState` — non-stop nodes pay no allocation for
/// either.
#[derive(Clone)]
pub(crate) struct EndMeasurements<Accumulator> {
...
/// Per-`EffectHandler` flow_measurement state.
///
/// Groups the flow_measurement-related fields that every processor
...
/// [`IncomingMeasurements`] and [`EndMeasurements`] and is `None` for nodes
/// that are not the corresponding endpoint of a flow_measurement.
///
/// All fields are `pub(crate)` so the local/shared `EffectHandler`
/// methods can read and write them directly through the
/// `flow_measurement.<field>` access path.
#[derive(Clone)]
pub(crate) struct FlowMeasurementState<Marker, Accumulator> {
    ...
    /// when flow_measurements are inactive on this pipeline; otherwise stays
    /// `Some(_)` once armed.
    pub last_send_marker: Marker,
    /// Whether this node is a flow measurement start node.
    pub is_start: bool,
    /// Whether this node is a flow measurement end node.
    pub is_end: bool,
    /// Whether any flow_measurement is configured in this pipeline.
    ...
    /// `None` when this node is not a flow_measurement start node — non-start
    /// nodes pay no allocation cost for the metric set or accumulator.
    pub incoming: IncomingMeasurements<Accumulator>,
    /// End-side measurements.
    pub end: EndMeasurements<Accumulator>,
}

/// Concrete `FlowMeasurementState` for the local (`!Send`) `EffectHandler`.
pub(crate) type LocalFlowMeasurementState =
    FlowMeasurementState<Rc<Cell<Option<Instant>>>, Cell<Mmsc>>;

/// Concrete `FlowMeasurementState` for the shared (`Send + Sync`)
/// `EffectHandler`.
pub(crate) type SharedFlowMeasurementState =
    FlowMeasurementState<Arc<Mutex<Option<Instant>>>, Arc<Mutex<Mmsc>>>;

impl Default for LocalFlowMeasurementState {
    fn default() -> Self {
        Self {
...
impl Default for SharedFlowMeasurementState {
    fn default() -> Self {
        Self {
...
/// Build flow_measurement state from the telemetry policy configuration.
///
/// Resolves flow_measurement start/stop node names to node indices, validates
...
///
/// FlowMeasurementes are explicit opt-in via the telemetry policy YAML and require
...
///
/// Returns `Err(Error::ConfigError(InvalidUserConfig))` if any flow_measurement
/// references an unknown node, has a non-processor endpoint, or overlaps
/// with another flow_measurement — these are configuration mistakes that should
/// fail pipeline startup rather than be silently skipped.
pub(crate) fn build_flow_measurement_state(
    telemetry_policy: &TelemetryPolicy,
    node_name_to_index: &HashMap<String, usize>,
    processor_indices: &HashSet<usize>,
    pipeline_context: &PipelineContext,
) -> Result<PipelineFlowMeasurementState, crate::error::Error> {
    ...
    for flow_config in &telemetry_policy.flow_measurements {
        ...
        let (Some(start_idx), Some(end_idx)) = (start_idx, end_idx) else {
            return Err(invalid_flow_measurement_config(format!(
                "flow measurement `{}` references unknown node(s): start=`{}`, end=`{}`",
        ...
        if !processor_indices.contains(&start_idx) || !processor_indices.contains(&end_idx) {
            return Err(invalid_flow_measurement_config(format!(
                "flow measurement `{}` start/end nodes must be processors: start=`{}`, end=`{}`",
                flow_config.name, flow_config.bounds.start_node, flow_config.bounds.end_node
            )));
        }
        ...
        {
            return Err(invalid_flow_measurement_config(format!(
                "flow measurement `{}` overlaps with another flow measurement (non-overlapping ranges only): start=`{}`, end=`{}`",
                flow_config.name, flow_config.bounds.start_node, flow_config.bounds.end_node
            )));
        }
        ...
        let signals_incoming_metric =
            flow_config.has(FlowMeasurement::SignalsIncoming).then(|| {
                pipeline_context
                    .metrics_registry()
        ...
        let signals_outgoing_metric =
            flow_config.has(FlowMeasurement::SignalsOutgoing).then(|| {
                pipeline_context
                    .metrics_registry()
    ...
    Ok(PipelineFlowMeasurementState {
...
fn invalid_flow_measurement_config(error: String) -> crate::error::Error {
...
/// `Instant`s for flow_measurement accumulation.
#[inline]
#[must_use]
...
impl PipelineFlowMeasurementState {
    /// Create an empty flow_measurement state (no flow_measurements configured).
    #[must_use]
    #[allow(dead_code)]
    pub fn empty() -> Self {
        Self {
    ...
    /// Returns `true` if any flow_measurements are configured.
    #[must_use]
    #[allow(dead_code)]
...
mod tests {
    ...
    fn one_flow_measurement_state() -> PipelineFlowMeasurementState {
    ...
    fn empty_state_is_inactive() {
        let state = PipelineFlowMeasurementState::empty();
        assert!(!state.is_active());
    }

    #[test]
    fn nonempty_state_is_active() {
        let state = one_flow_measurement_state();
        assert!(state.is_active());
    }

    #[test]
    fn direct_record_increments_mmsc() {
        let mut state = one_flow_measurement_state();
    ...
    // -- build_flow_measurement_state validation tests --

    use otap_df_config::policy::{
        FlowBounds, FlowMeasurement, FlowMeasurementConfig, TelemetryPolicy,
    };

    fn policy_with(flow_measurements: Vec<FlowMeasurementConfig>) -> TelemetryPolicy {
        TelemetryPolicy {
            flow_measurements,
    ...
    fn sw(name: &str, start: &str, stop: &str) -> FlowMeasurementConfig {
        FlowMeasurementConfig {
            name: name.to_string(),
            bounds: FlowBounds {
    ...
    /// message mentions the given flow_measurement name.
    ...
    #[test]
    fn valid_flow_measurement_is_registered() {
        let (ctx, _) = test_pipeline_ctx();
    ...
    fn unknown_node_is_rejected() {
        ...
        let err = build_flow_measurement_state(&policy, &names, &procs, &ctx)
            .err()
            .expect("expected Err");
    ...
    fn shared_start_node_is_rejected() {
        ...
        // Two flow_measurements share "a" as start node.
    ...
    fn shared_end_node_is_rejected() {
        let (ctx, _) = test_pipeline_ctx();
        let (names, procs) = test_maps(&["a", "b", "c", "d"], &[]);
        // Two flow_measurements share "d" as stop node.
    ...
    #[test]
    fn disjoint_flow_measurements_are_both_registered() {
        let (ctx, _) = test_pipeline_ctx();
        let (names, procs) = test_maps(&["a", "b", "c", "d"], &[]);
