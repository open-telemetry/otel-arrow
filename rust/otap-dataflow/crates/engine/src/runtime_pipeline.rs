    ...
    RuntimePipeline<PData>
{
    ...
    pub fn run_forever(
        ...
        rt.block_on(async {
            // Inner async block isolates the loop's `return Err(...)`
            // short-circuits from the outer cleanup, giving us an
            // "async finally" shape: cleanup always runs, then the
            // loop's result is propagated.
            let loop_result: Result<Vec<_>, Error> = async {
                let mut task_results = Vec::new();
                loop {
                    // `biased;`: prefer data-path completions when both arms
                    // are simultaneously ready. Functionally optional; kept
                    ...
                    // Once the data path is drained, broadcast `Shutdown` and
                    // exit the loop so the bounded `drain_until_deadline`
                    // below owns the wait for remaining extension tasks.
                    if futures.is_empty() {
            ...
            // "Async finally": unconditional cleanup that runs on both the
            // normal and error paths. `broadcast_shutdown` is idempotent
            // (latched by `shutdown_broadcast_fired`), so a no-op on the
            // normal path; on the error path it ensures already-started
            // extensions still receive `Shutdown` before the pipeline exits.
            extension_lifecycle.broadcast_shutdown().await;
