...
impl QuiverEngine {
    ...
    pub fn force_drop_oldest_pending_segments(&self) -> usize {
        ...
        for seq in &to_drop {
            ...
            if let Ok((bundles, items, items_per_slot)) =
                self.segment_store.segment_drop_counts(*seq)
            {
                bundles_dropped += bundles as u64;
                items_dropped += items;
                for (&slot, &count) in items_per_slot.iter() {
                    *items_dropped_per_slot.entry(slot).or_insert(0) += count;
                }
            }
            if let Err(e) = self.segment_store.delete_segment(*seq) {
    ...
    pub fn cleanup_expired_segments(&self) -> std::result::Result<usize, SubscriberError> {
        ...
        for seq in &expired_segments {
                ...
                self.segment_store.segment_drop_counts(*seq)
            {
                ...
                items_expired += items;
                for (&slot, &count) in items_per_slot.iter() {
                    *items_expired_per_slot.entry(slot).or_insert(0) += count;
                }
            }
