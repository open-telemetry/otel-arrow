...
(`core-nodes`, `dev-nodes`, `contrib-nodes`, `otap`) and cross-checks the
compiler-resolved
`otel_arrow_dfe_engine::inventory::COMPONENT_INVENTORY` slice against the
committed `components-baseline.json`.
