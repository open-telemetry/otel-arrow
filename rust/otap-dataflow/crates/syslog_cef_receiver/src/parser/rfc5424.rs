...
mod tests {
    ...
    fn test_rfc5424_parsing() {
        ...
        assert_eq!(result.timestamp, Some(b"2003-10-11T22:14:15.003Z".as_slice()));
        ...
        assert_eq!(result.message, Some(b"BOM'su root' failed for lonvick on /dev/pts/8".as_slice()));
    }
    ...
    fn test_structured_data_rfc5424() {
        ...
        assert_eq!(result.structured_data, Some(b"[exampleSDID@32473 iut=\"3\" eventSource=\"Application\" eventID=\"1011\"]".as_slice()));
        assert_eq!(result.message, Some(b"BOMAn application event log entry".as_slice()));
    }
    ...
    fn test_rfc5424_only_structured_data() {
        ...
        assert_eq!(result.structured_data, Some(b"[id@123 key=\"value\"]".as_slice()));
        assert_eq!(result.message, None);
    }
    ...
    fn test_rfc5424_multiple_structured_data() {
        ...
        assert_eq!(result.structured_data, Some(b"[id1@123 key1=\"val1\"][id2@456 key2=\"val2\"]".as_slice()));
    ...
    fn test_rfc5424_multiple_structured_data_with_spaces() {
        ...
        assert_eq!(result.structured_data, Some(b"[id1@123 key1=\"val1\"] [id2@456 key2=\"val2\"] [id3@789 key3=\"val3\"]".as_slice()));
        assert_eq!(result.message, Some(b"Message text".as_slice()));
    }
    ...
    fn test_byte_slice_to_string_conversion() {
        ...
        assert_eq!(std::str::from_utf8(result.timestamp.unwrap()).unwrap(), "2003-10-11T22:14:15.003Z");
        assert_eq!(std::str::from_utf8(result.hostname.unwrap()).unwrap(), "mymachine.example.com");
        assert_eq!(std::str::from_utf8(result.app_name.unwrap()).unwrap(), "su");
        assert_eq!(std::str::from_utf8(result.msg_id.unwrap()).unwrap(), "ID47");
        assert_eq!(std::str::from_utf8(result.message.unwrap()).unwrap(), "BOM'su root' failed for lonvick on /dev/pts/8");
    }
}
