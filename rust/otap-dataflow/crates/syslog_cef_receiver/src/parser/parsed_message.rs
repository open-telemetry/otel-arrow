...
use crate::parser::{rfc5424::Rfc5424Message, rfc3164::Rfc3164Message, cef::CefMessage};
...
impl ParsedSyslogMessage<'_> {
                ...
                std::str::from_utf8(ts).ok().and_then(|timestamp_str| {
                    ...
                    if let Ok(naive_dt) = NaiveDateTime::parse_from_str(&full_timestamp, "%Y %b %d %H:%M:%S") {
                        ...
                        if let Some(local_dt) = Local.from_local_datetime(&naive_dt).single() {
                            return Some(local_dt.with_timezone(&Utc).timestamp_nanos_opt().unwrap_or(0) as u64);
                        }
                    }
    ...
    #[must_use]
    pub(crate) fn add_attribues_to_arrow(&self, log_attributes_arrow_records: &mut AttributesRecordBatchBuilder<u16>) -> u16 {
        ...
        match self {
            ParsedSyslogMessage::Rfc5424(msg) => {
                ...
                if let Some(hostname) =  msg.hostname {
                    ...
                    log_attributes_arrow_records.append_str(std::str::from_utf8(hostname).unwrap_or_default());
                    attributes_count += 1;
                }

                if let Some(appname) = msg.app_name {
                    log_attributes_arrow_records.append_key(SYSLOG_APP_NAME);
                    log_attributes_arrow_records.append_str(std::str::from_utf8(appname).unwrap_or_default());
                    attributes_count += 1;
                }

                if let Some(proc_id) = msg.proc_id {
                    log_attributes_arrow_records.append_key(SYSLOG_PROCESS_ID);
                    log_attributes_arrow_records.append_int(std::str::from_utf8(proc_id).unwrap_or_default().parse::<i64>().unwrap_or(0));
                    attributes_count += 1;
                }

                if let Some(msg_id) = msg.msg_id {
                    log_attributes_arrow_records.append_key(SYSLOG_MSG_ID);
                    log_attributes_arrow_records.append_str(std::str::from_utf8(msg_id).unwrap_or_default());
                    attributes_count += 1;
                }

                if let Some(structured_data) = &msg.structured_data {
                    log_attributes_arrow_records.append_key(SYSLOG_STRUCTURED_DATA);
                    log_attributes_arrow_records.append_str(std::str::from_utf8(structured_data).unwrap_or_default());
                    attributes_count += 1;
                }

                if let Some(message) = msg.message {
                    log_attributes_arrow_records.append_key(SYSLOG_MESSAGE);
                    log_attributes_arrow_records.append_str(std::str::from_utf8(message).unwrap_or_default());
                    attributes_count += 1;
                }
            ...
            ParsedSyslogMessage::Rfc3164(msg) => {
                ...
                if let Some(tag) = msg.tag {
                    ...
                    log_attributes_arrow_records.append_str(std::str::from_utf8(tag).unwrap_or_default());
                    attributes_count += 1;
                }

                if let Some(content) = msg.content {
                    log_attributes_arrow_records.append_key(SYSLOG_CONTENT);
                    log_attributes_arrow_records.append_str(std::str::from_utf8(content).unwrap_or_default());
                    attributes_count += 1;
                }
            ...
            ParsedSyslogMessage::Cef(msg) => {
                ...
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.device_vendor).unwrap_or_default());

                log_attributes_arrow_records.append_key(CEF_DEVICE_PRODUCT);
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.device_product).unwrap_or_default());

                log_attributes_arrow_records.append_key(CEF_DEVICE_VERSION);
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.device_version).unwrap_or_default());

                log_attributes_arrow_records.append_key(CEF_SIGNATURE_ID);
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.signature_id).unwrap_or_default());

                log_attributes_arrow_records.append_key(CEF_NAME);
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.name).unwrap_or_default());

                log_attributes_arrow_records.append_key(CEF_SEVERITY);
                log_attributes_arrow_records.append_str(std::str::from_utf8(msg.severity).unwrap_or_default());

                for (key, value) in &msg.extensions {
                    log_attributes_arrow_records.append_key(std::str::from_utf8(key).unwrap_or_default());
                    log_attributes_arrow_records.append_str(std::str::from_utf8(value).unwrap_or_default());
                    attributes_count += 1;
                }
...
mod tests {
    ...
    fn test_parsed_syslog_message_timestamp_rfc3164() {
        ...
        if let Ok(naive_dt) = NaiveDateTime::parse_from_str(&full_timestamp, "%Y %b %d %H:%M:%S") {
            if let Some(local_dt) = Local.from_local_datetime(&naive_dt).single() {
                let expected_nanos = local_dt.with_timezone(&Utc).timestamp_nanos_opt().unwrap_or(0) as u64;
                assert_eq!(timestamp_nanos, expected_nanos);
            }
        }
    }
    ...
    fn test_parsed_syslog_message_input() {
        ...
        assert_eq!(input_str, "<34>1 2003-10-11T22:14:15.003Z host app - - - Test message");
    }
}
