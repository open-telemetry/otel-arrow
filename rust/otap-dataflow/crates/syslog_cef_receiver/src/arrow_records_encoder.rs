use otel_arrow_rust::{
    encode::record::{
        attributes::AttributesRecordBatchBuilder,
        logs::LogsRecordBatchBuilder,
    },
    otap::{Logs, OtapArrowRecords},
    otlp::logs::logs_from,
    proto::opentelemetry::{arrow::v1::ArrowPayloadType, common::v1::any_value::Value},
};

use otap_df_otap::encoder::error::Result;

use crate::parser::{parsed_message::ParsedSyslogMessage, parse};
...
impl ArrowRecordsBuilder {
    ...
    {
        self.logs.append_time_unix_nano(syslog_message.timestamp().map(|v| v as i64));
        self.logs.append_observed_time_unix_nano(None);
        self.logs.append_schema_url(None);

        let (severity_number, severity_text) = syslog_message.severity().unwrap_or((0, "UNSPECIFIED"));
    ...
    pub(crate) fn build(mut self) -> Result<OtapArrowRecords> {
        ...
        self.logs.resource.append_dropped_attributes_count_n(0, log_record_count);
        ...
        self.logs.scope.append_dropped_attributes_count_n(0, log_record_count);
...
mod tests {
    ...
    fn test_rfc5424_syslog_to_arrow_and_back() {
        ...
        let log1_attrs: std::collections::HashMap<String, String> = log1.attributes.iter()
            .map(|kv| (kv.key.clone(), 
        ...
        assert_eq!(log1_attrs.get("syslog.host_name"), Some(&"mymachine.example.com".to_string()));
        assert_eq!(log1_attrs.get("syslog.app_name"), Some(&"su".to_string()));
        assert_eq!(log1_attrs.get("syslog.msg_id"), Some(&"ID47".to_string()));
        assert_eq!(log1_attrs.get("syslog.message"), Some(&"'su root' failed for lonvick on /dev/pts/8".to_string()));

        // Ensure no unexpected attributes are present (exactly 7 attributes expected)
        assert_eq!(log1.attributes.len(), 7);

        // Verify second log record with structured data
        let log2_attrs: std::collections::HashMap<String, String> = log2.attributes.iter()
            .map(|kv| (kv.key.clone(), 
                       match &kv.value.as_ref().unwrap().value.as_ref().unwrap() {
                           Value::StringValue(s) => s.clone(),
                           Value::IntValue(i) => i.to_string(),
        ...
        assert_eq!(log2_attrs.get("syslog.host_name"), Some(&"host.example.com".to_string()));
        assert_eq!(log2_attrs.get("syslog.app_name"), Some(&"myapp".to_string()));
        assert_eq!(log2_attrs.get("syslog.process_id"), Some(&"1234".to_string()));
        assert_eq!(log2_attrs.get("syslog.msg_id"), Some(&"ID123".to_string()));
        assert_eq!(log2_attrs.get("syslog.structured_data"), Some(&"[exampleSDID@32473 iut=\"3\" eventSource=\"Application\" eventID=\"1011\"]".to_string()));
        assert_eq!(log2_attrs.get("syslog.message"), Some(&"Application started successfully".to_string()));
    ...
    fn test_rfc3164_syslog_to_arrow_and_back() {
        ...
        assert_eq!(log1.body.as_ref().unwrap().value.as_ref().unwrap(), 
                   &Value::StringValue(
                       "<34>Oct 11 22:14:15 mymachine su: 'su root' failed for lonvick on /dev/pts/8".to_string()
                   ));
        ...
        assert_eq!(log2.body.as_ref().unwrap().value.as_ref().unwrap(),
                   &Value::StringValue(
                       "<165>Feb  5 17:32:18 hostname app[1234]: Application started successfully".to_string()
                   ));
        ...
        assert_eq!(log1_attrs.get("syslog.facility"), Some(&"4".to_string()));
        assert_eq!(log1_attrs.get("syslog.severity"), Some(&"2".to_string()));
        assert_eq!(log1_attrs.get("syslog.host_name"), Some(&"mymachine".to_string()));
        ...
        assert_eq!(log2_attrs.get("syslog.facility"), Some(&"20".to_string()));
        assert_eq!(log2_attrs.get("syslog.severity"), Some(&"5".to_string()));
        assert_eq!(log2_attrs.get("syslog.host_name"), Some(&"hostname".to_string()));
    ...
    fn test_cef_to_arrow_and_back() {
        ...
        assert_eq!(log1_attrs.get("cef.device_vendor"), Some(&"Security".to_string()));
        assert_eq!(log1_attrs.get("cef.device_product"), Some(&"threatmanager".to_string()));
        assert_eq!(log1_attrs.get("cef.device_version"), Some(&"1.0".to_string()));
        assert_eq!(log1_attrs.get("cef.signature_id"), Some(&"100".to_string()));
        assert_eq!(log1_attrs.get("cef.name"), Some(&"worm successfully stopped".to_string()));
        ...
        assert_eq!(log2_attrs.get("cef.device_vendor"), Some(&"ArcSight".to_string()));
        assert_eq!(log2_attrs.get("cef.device_product"), Some(&"ArcSight".to_string()));
        assert_eq!(log2_attrs.get("cef.device_version"), Some(&"2.4.1".to_string()));
        assert_eq!(log2_attrs.get("cef.signature_id"), Some(&"400".to_string()));
        assert_eq!(log2_attrs.get("cef.name"), Some(&"Successful Login".to_string()));
        ...
        // // Verify third CEF log record attributes (complex extensions)
        // let log3_attrs: std::collections::HashMap<String, String> = log3.attributes.iter()
        //     .map(|kv| (kv.key.clone(), 
        //                match &kv.value.as_ref().unwrap().value.as_ref().unwrap() {
        //                    Value::StringValue(s) => s.clone(),
        //                    Value::IntValue(i) => i.to_string(),
