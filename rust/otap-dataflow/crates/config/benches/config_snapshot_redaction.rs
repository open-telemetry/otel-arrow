...
fn enforce_targets(results: &[WorkloadResult]) {
    ...
    assert!(
        typed_typical.latency_ns.p99 < 100_000,
        "typed typical p99 must remain below 100 us"
    );
    assert!(
        typed_large.latency_ns.p99 < 4_000_000,
        "typed large p99 must remain below 4 ms"
    );
    assert!(
