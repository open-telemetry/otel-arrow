...
- `ComponentInventoryArgs`: parsed `#[component_inventory(...)]` arguments
- `Category`: controlled component categories and their URN segments
- `Protocol`: normalized protocol vocabulary with a `Custom` fallback
- `Auth`: normalized authentication vocabulary with a `Custom` fallback
...
This is an implementation crate shared by `otel-arrow-dfe-engine-macros` and
the component inventory tooling. Applications should not depend on it
directly.
