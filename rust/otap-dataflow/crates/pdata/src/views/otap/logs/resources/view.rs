...
use crate::views::otap::common::{
    OtapAttributeIter, OtapAttributeView, build_attribute_index, ensure_transport_id_decoded,
...
impl<'a> OtapLogsResourcesView<'a> {
    pub(super) fn new(
        ...
        ensure_transport_id_decoded(ArrowPayloadType::Logs, logs_batch, RESOURCE_ID_COL_PATH)?;
        ensure_transport_id_decoded(
...
impl<'a> OtapLogsResource<'a> {
    ...
    #[must_use]
    pub fn resource(&self) -> OtapLogsResourceView<'_> {
        OtapLogsResourceView {
            view: self.view,
...
pub struct OtapLogsResourceView<'a> {
    view: &'a OtapLogsResourcesView<'a>,
    resource_id: u16,
}

impl<'a> ResourceView for OtapLogsResourceView<'a> {
    type Attribute<'att>
