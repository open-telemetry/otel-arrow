...
mod test {
    ...
    fn test_metrics_proto_encode() {
        ...
        let exp_hist_dp_exemplars = RecordBatch::try_new(
            ...
            vec![
                ...
                Arc::new(UInt32Array::from_iter_values([0])),
                Arc::new(
                    TimestampNanosecondArray::from_iter_values([1])
                        .with_timezone(TIMESTAMP_TIME_ZONE),
                ),
    ...
    #[test]
    fn test_metrics_proto_encode_missing_root_no_payloads_returns_ok() {
        // A missing root metrics payload is semantically equivalent to 0 rows:
        // encode should succeed and produce an empty result rather than erroring.
        let mut otap_batch = OtapArrowRecords::Metrics(Metrics::default());
        let mut result_buf = ProtoBuffer::default();
        let mut encoder = MetricsProtoBytesEncoder::new();
        ...
        let decoded = MetricsData::decode(result_buf.as_ref()).unwrap();
        assert!(decoded.resource_metrics.is_empty());
    }

    #[test]
    fn test_metrics_proto_encode_missing_root_with_child_payloads_returns_ok() {
        // Even when non-root child payloads are present, a missing root metrics
        ...
        let mut otap_batch = OtapArrowRecords::Metrics(Metrics::default());
        // Set child payloads but deliberately omit the root metrics payload.
        otap_batch
            .set(ArrowPayloadType::MetricAttrs, attrs_record_batch.clone())
            .unwrap();
        otap_batch
