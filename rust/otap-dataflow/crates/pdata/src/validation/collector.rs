...
impl CollectorProcess {
    ...
    pub async fn shutdown(&mut self) -> Result<()> {
        #[cfg(unix)]
        {
            ...
            kill(Pid::from_raw(pid as i32), Signal::SIGTERM)
                .map_err(|e| Error::SignalNotDelivered { source: e })?;
        }
        ...
        let status = self.process.wait().await.map_err(|e| Error::InputOutput {
            source: e,
            desc: "wait",
        })?;
    ...
    pub async fn start<T: AsRef<Path>>(collector_path: T, config_content: &str) -> Result<Self> {
        ...
        let mut file = fs::File::create(&config_path).map_err(|e| Error::InputOutput {
            source: e,
            desc: "create",
        })?;
        ...
        let stdout = process
            .stdout
            .take()
            .ok_or_else(|| Error::FileNotAvailable { desc: "stdout" })?;

        let stderr = process
            .stderr
            .take()
            .ok_or_else(|| Error::FileNotAvailable { desc: "stderr" })?;
        ...
        tokio::time::timeout(timeout_duration, ready_rx)
            .await
            .map_err(|e| Error::ReadyTimeout { source: e })?
            .map_err(|e| Error::ChannelClosed { source: e })?;
...
pub async fn run_test<I, O, F>(test_logic: F) -> Result<()>
where
...
{
    // Pick a free loopback port for the collector receiver.
